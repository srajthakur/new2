...
def index(request,myid):
    ...
    c = Cart.objects.filter(uid=myid)
    ...
    params = {'ns': ns, 'range': range(1,ns), 'xv': p,'n':n,'myid':myid}
...
def quickview(request,myid,pid):
    p = product.objects.filter(id=pid)
    c = Cart.objects.filter(uid=myid)
    n = len(c)
    pa={'xv':p[0],'n':n,'myid':myid}

    if request.method=="POST":
       desc = request.POST.get('id', '')
       k = Cart.objects.filter(cartid=pid,uid=myid)

       if len(k)==0:
         l = Cart.objects.all()
         l=Cart(uid=myid,cartid=pid,cartname=p[0].pname,cartdecs=p[0].decs,cartimage=p[0].image,cartprice=p[0].price)
         l.save()
...
def cart(request,myid):
    ...
    if request.method == "POST":
        ...
        Cart.objects.get(uid=myid,cartid=o).delete()
    ...
    n = len(c)
    if n==0:
        pa = {'xv': c, 'n': n,  'myid': myid}
        return render(request, 'shop/ecart.html' ,pa)

    price=0
    for i in range(0,n):
        price=price+c[i].cartprice
    pa = {'xv': c, 'n': n,'price':price,'myid':myid}
...
def adress(request,myid):
    c = Cart.objects.filter(uid=myid)
    n = len(c)

    a = Adress.objects.filter(uid=myid)
    pa = {'xv': a[0],'n':n,'myid':myid}
    ...
    if request.method=='POST':
        ...
        phone = request.POST.get('phone', '')
        adress=Adress(uid=myid,name=name,houseno=hno,streetadress=street,pincode=pin,city=city,distric=distric,state=state,phone=phone)
        adress.save()
...
def review(request,myid):
    c = Cart.objects.filter(uid=myid)
    n = Adress.objects.filter(uid=myid)
    pa = {'xv': c, 'n': n[0],'myid':myid}
